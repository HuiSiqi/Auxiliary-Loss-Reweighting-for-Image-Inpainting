...
def weighted_generator_loss_func(
        mask, output, ground_truth, edge, output_pred,
        vgg_comp, vgg_output, vgg_ground_truth,
        projected_image, projected_edge,
        output_edge,plweight,slweight):
    l1 = nn.L1Loss()
    criterion = nn.BCELoss()
    ...
    # ---------------
    loss_perceptual = 0.0
    loss_perceptual_terms = []
    for i in range(3):
        pl1 = l1(vgg_output[i], vgg_ground_truth[i])
        pl2 = l1(vgg_comp[i], vgg_ground_truth[i])
        pl = (pl1+pl2).mean()
        loss_perceptual += plweight[i]*pl
        loss_perceptual_terms.append(pl)
    ...
    loss_style_term = []
    for i in range(3):
        sl1 = l1(gram_matrix(vgg_output[i]), gram_matrix(vgg_ground_truth[i]))
        sl2 = l1(gram_matrix(vgg_comp[i]), gram_matrix(vgg_ground_truth[i]))
        sl = (sl1+sl2).mean()
        loss_style += slweight[i]*sl
        loss_style_term.append(sl)
    ...
    return {
        ...
        'loss_adversarial': loss_adversarial.mean(),
        'loss_intermediate': loss_intermediate.mean(),
        'loss_style_terms':loss_style_term,
        'loss_perceptual_terms':loss_perceptual_terms
    }
